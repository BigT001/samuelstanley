...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!project) {
    return { title: "Project Not Found | Samuel Stanley" };
  }

  const title = `${project.title} Case Study | Samuel Stanley — Full Stack Developer`;
  const description = `${project.desc} Built by Samuel Stanley, Full Stack Developer based in Lagos, Nigeria.`;
  ...
  return {
    ...
    openGraph: {
      ...
      images: [{ url: "/og-image.png", width: 1200, height: 630, alt: `${project.title} — Case Study` }],
    },
    twitter: {
